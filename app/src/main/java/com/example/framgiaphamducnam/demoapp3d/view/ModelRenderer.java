...
import com.example.framgiaphamducnam.demoapp3d.model.Object3D;
...
import com.example.framgiaphamducnam.demoapp3d.model.Object3DImpl;
import com.example.framgiaphamducnam.demoapp3d.services.SceneLoader;
import com.example.framgiaphamducnam.demoapp3d.util.GLUtil;
...
public class ModelRenderer implements GLSurfaceView.Renderer {
	...
	private float[] mEye = { (float) 0.0, (float) 0.0, (float) 6.0 };
	...
	public void onSurfaceChanged(GL10 unused, int width, int height) {
		...
		Matrix.setLookAtM(modelViewMatrix, 0, mEye[0], mEye[1], mEye[2], camera.xView, camera.yView,
	...
	public void onDrawFrame(GL10 unused) {
		...
		for (int i=0; i<objects.size(); i++) {
			...
			try {
				...
				if (textures!=null && textures.get(objData.getTextureData())!=null){
				}
...
//							wireframe.centerAndScale(5.0f);
	...
	public void move(){
		//float f=far/near;
		////
		//Matrix.scaleM(mvpMatrix, 0, mvpMatrix, 0,
		//		size*2*f*width/height, size*2*f, 1.0f);
		Matrix.translateM(mvpMatrix, 0, 0.0f, 0.0f, -3.0f);
	}
	...
	public void translateNew(float dx, float dy){
		float d1 = (float) 0.0;
		float d2 = (float) 10.2;
		int max = Math.max(getWidth(), getHeight());
		d1 = (float)(d1 / max * Math.PI * 2);
		d2 = (float)(d2 / max * Math.PI * 2);
		if (camera == null){
			camera = new Camera();
		}
		camera.translateCamera(dx, dy);
	}

	public void zoomIn(){
		if (camera == null){
			camera = new Camera();
			camera.setScene(main.getModelActivity().getScene());
		}
		camera.MoveCameraZ((float)0.3);
	}

	public void changePosition(float y){
		Matrix.setLookAtM(modelViewMatrix, 0, mEye[0], mEye[1], mEye[2], camera.xView, y,
				camera.zView, camera.xUp, camera.yUp, camera.zUp);
	}
