package com.example.framgiaphamducnam.demoapp3d.model;
...
import com.example.framgiaphamducnam.demoapp3d.services.WavefrontLoader.*;
import com.example.framgiaphamducnam.demoapp3d.services.collada.loader.ColladaLoader;
import com.example.framgiaphamducnam.demoapp3d.services.stl.STLLoader;
import com.example.framgiaphamducnam.demoapp3d.services.wavefront.WavefrontLoader2;
import com.example.framgiaphamducnam.demoapp3d.util.math.Math3DUtils;
...
public final class Object3DBuilder {
	...
	public Object3D getDrawer(Object3DData obj, boolean usingTextures, boolean usingLights) throws
            IOException {
	...
	public static Object3DData generateArrays(AssetManager assets, Object3DData obj) throws
            IOException {
		...
		final FloatBuffer
                vertexArrayBuffer = createNativeByteBuffer(faces.getVerticesReferencesCount() * 3 * 4).asFloatBuffer();
		...
		final FloatBuffer
                vertexNormalsArrayBuffer = createNativeByteBuffer(faces.getSize() * 3 * 3 * 4).asFloatBuffer();;
		...
		if (materials != null && !faceMats.isEmpty()) {
			...
			for (int i = 0; i < faces.getSize(); i++) {
				if (faceMats.findMaterial(i) != null) {
					...
					//if (materials.getMaterial(faceMats.findMaterial(i)).equals("young_asian_male")){
					//	Log.e("Object3DBuilder", "generateArrays:  -----> 1");
					//} else if (materials.getMaterial(faceMats.findMaterial(i)).equals
					//		("Eye_brown")) {
					//	Log.e("Object3DBuilder", "generateArrays:  -----> 2");
					//}
					if (mat != null) {
		...
		if (materials != null && !materials.materials.isEmpty()) {
			...
			if (texture != null) {
				if (obj.getCurrentDir() != null) {
					...
					Log.i("Object3DBuilder", "Loading texture 111'" + file + "'...");
					...
					File file = new File(obj.getCurrentDir(), texture);
					String assetResourceName = obj.getAssetsDir() + "/" + file.getName();
		...
		if (drawBuffer != null) {
			...
			for (int i = 0; i < vertexBuffer.capacity() / /* COORDS_PER_VERTEX */ 3 / /* VERTEX_PER_FACE */3; i++) {
				...
				@SuppressWarnings("unused") String
                        normal = new StringBuilder().append(normalLine[0][0]).append(",").append(normalLine[0][1])
...
class BoundingBox {
	 ...
	 */
	public BoundingBox(com.example.framgiaphamducnam.demoapp3d.entities.BoundingBox box, float[] color) {
		// initialize vertex byte buffer for shape coordinates
	...
	private void calculateVertex(com.example.framgiaphamducnam.demoapp3d.entities.BoundingBox box) {
		//@formatter:off
